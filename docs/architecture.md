...
Cookie、server-function 缓存、邮箱与摘要缓存和记录页缓存都不跨账号共享。即使两个账号指向同一工作区，也会分别访问和缓存。只有不含凭据和数据的异步信号量由账号共享，将全部账号合计的官网并发请求限制为 8 个。
...
每个账号的刷新过程使用一个异步 single-flight 锁。不同账号可以并行刷新；同一账号的自动刷新、手工刷新和 API 调用会被合并，避免冲击官网。并发的多个 `refresh=true` 请求会记录进入锁前的刷新代际；等待期间若已有请求完成刷新，后续请求直接复用该代结果，不会逐个再次绕过缓存。非并发的手工刷新仍会按请求语义重新抓取。

摘要和当前记录页都缺失时，二者通过 `tokio::join!` 并行抓取。

官网 HTML、bundle 和 RPC 响应分别有 4 MiB、8 MiB 和 4 MiB 上限。响应体按网络 chunk 增量读取，每次扩容前检查上限，因此没有 `Content-Length` 的分块响应也无法先占满内存。记录页若超过官网固定的 50 条会作为协议变化拒绝，而不是继续解析。

Salvo 最多同时处理 256 个客户端连接。进程收到 SIGTERM 或 Ctrl-C 后停止接受新连接，等待现有请求最多 10 秒；Compose 的 `stop_grace_period` 为 15 秒，为应用清理和容器运行时调度保留余量。
...
| 非回环 HTTP `base_url` 或带凭据的 URL | 启动时拒绝配置 |
