...
1. 服务固定读取进程工作目录中的 `config.json`，不读取命令行参数、环境变量或旧 `account.json`。
2. 从旧版本升级时，将有效配置迁移到 `config.json`，确认服务可启动后安全删除旧 `account.json`。
3. 公网部署必须设置非空 `server.panel_key`，推荐使用至少 32 位随机 Key：
...
5. 不要直接暴露后端端口。使用 HTTPS 反向代理，并让后端只监听回环地址或 Docker 内部网络。
